...
const UlWithNumber = ({ ul }: { ul: { ulHead: string; ulP: string }[] }) => {
  ...
  return (
    <div>
      <ul
        ref={ulRef}
        className={`${locale === "ar" ? "pr-10" : "pl-10" } list-decimal mb-5  transition-all duration-500 overflow-hidden`}
        style={{ height: showAll ? `${ulRef.current?.scrollHeight}px` : collapsedHeight }}
      >
        {ul.map((ele, ind) => (
          <li key={ind} className="mb-4">
            {ele.ulHead && <h2 className="font-normal mb-2 text-lg lg:text-xl text-custom-black">{ele.ulHead}</h2>}
